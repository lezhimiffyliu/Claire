...
(SQLAlchemy over in-memory SQLite) and Supabase JWT identity.
...
import api
import app.persistence.base as db_base
import app.persistence.models  # noqa: F401
from app.persistence.base import Base
from app.persistence.models import AttemptRow, TeachingStateRow
...
def _as_user(monkeypatch, user_id=AUTHED_USER):
    # Identity is resolved from a verified Supabase JWT via get_optional_auth
    # (async, returns (user_id, client)). Stub it to simulate an authed user.
    async def _fake_auth(request):
        return user_id, None
    monkeypatch.setattr(api, "get_optional_auth", _fake_auth)
...
# --------------------------------------------------------------------------- #
# Supabase JWT verification (integration: real get_optional_auth path)
# --------------------------------------------------------------------------- #
def test_supabase_authenticated_request_is_not_anonymous(client, db_sf, monkeypatch):
    """A valid Supabase JWT resolves to the real user through the ACTUAL
    get_optional_auth -> verify_jwt path (Supabase client mocked, not the endpoint)
    and persists under that user id — proving authenticated requests are not
    silently treated as anonymous."""
    import app.auth.api_auth as api_auth

    supa_user = "supabase_user_xyz"
    user_obj = type("U", (), {"id": supa_user})()
    fake_client = MagicMock()
    fake_client.auth.get_user.return_value = type("R", (), {"user": user_obj})()
    # Patch the Supabase client factory, NOT the endpoint's identity function.
    monkeypatch.setattr(api_auth, "_get_anon_client", lambda: fake_client)
    monkeypatch.setattr(api_auth, "get_authenticated_client_from_token", lambda tok: None)
    ...
    resp = client.post(
        "/api/attempt",
        json={"problem_id": "q_deriv_1", "answer": "3*x**2", "attempt_session_id": "sess-supa"},
        headers={"Authorization": "Bearer valid.supabase.jwt"},
    )
    data = resp.json()
    assert data["persisted"] is True
    fake_client.auth.get_user.assert_called_once()  # the JWT was actually verified
    with db_sf() as s:
        attempts = s.execute(
            select(AttemptRow).where(AttemptRow.user_id == supa_user)
        ).scalars().all()
        assert len(attempts) == 1  # persisted under the resolved user, not "anonymous"

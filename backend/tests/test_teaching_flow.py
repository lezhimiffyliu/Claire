...
class TestBuildTeachingContext:
    ...
    def test_basic_context_structure(self):
        ...
        from app.grading.vision_analyzer import (
...
class TestAnalyzeWithVerifier:
    ...
    def _mock_alignment(self):
        # These tests exercise the verifier wiring, not the (heuristic/LLM)
        # problem-alignment gate — force alignment so the verifier path runs.
        with patch(
            "app.grading.alignment_checker.check_problem_alignment",
            return_value=MagicMock(is_aligned=True),
        ):
            yield
...
class TestVerifierIntegration:
    ...
    def test_verifier_type_detected(self):
        ...
        from app.grading.verifier import verify_answer
